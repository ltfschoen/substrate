	...
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		fn set_fee(origin,
			...
			<Registrars<T>>::mutate(|rs|
				rs.get_mut(index as usize)
					.and_then(|x| x.as_mut())
					.and_then(|r| if r.account == who { r.fee = fee; Some(None.into()) } else { None })
		...
		fn set_account_id(origin,
			...
			<Registrars<T>>::mutate(|rs|
				rs.get_mut(index as usize)
					.and_then(|x| x.as_mut())
					.and_then(|r| if r.account == who { r.account = new; Some(None.into()) } else { None })
					.ok_or_else(|| Error::<T>::InvalidIndex.into())
			)
		}
		...
		fn set_fields(origin,
			...
			<Registrars<T>>::mutate(|rs|
				rs.get_mut(index as usize)
					.and_then(|x| x.as_mut())
					.and_then(|r| if r.account == who { r.fields = fields; Some(None.into()) } else { None })
					.ok_or_else(|| Error::<T>::InvalidIndex.into())
			)
		}
